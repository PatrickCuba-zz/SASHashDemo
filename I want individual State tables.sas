Data Customer_Detail;
	...
	If _N_=1 Then Do;
		...
		Declare Hash EachStateBal(Ordered: "a", Multidata: "Y");
		EachStateBal.DefineKey('State');
		EachStateBal.DefineData('State', 'StateBal');
		EachStateBal.DefineDone();
	...
	* if rc=0; * To return matches only * ;
	...
	If End Then Do;
		...
		Do Until(Rc ne 0);
			Rc=EachStateBal.Clear();
			Rc=EachStateBal.Replace();
			Rc=EachStateBal.Output(Dataset: Compress("Work.State_"||State));
			Rc=ByStateIter.Next();
		End;
	End;
